    ...
    const conversationData = {
      ...
      messages: [
        ...
        },
        {
          role: 'assistant',
          content:
            "Bien sûr ! Je serais ravi de t'aider à créer un site web. Peux-tu me donner plus de détails sur le type de site que tu souhaites créer ?",
          timestamp: new Date().toISOString()
        }
      ]
    };
...
function loadRecentLinks() {
  ...
  historyList.innerHTML = recentLinks
    .map(
      (link) => `
...
function downloadFile() {
  ...
  const format = document.getElementById('exportFormat')?.value || 'pdf';
  const extension =
    format === 'pdf' ? 'pdf' : format === 'docx' ? 'docx' : format === 'md' ? 'md' : 'txt';
...
function loadHistory() {
  ...
  if (history.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="5" class="text-muted" style="text-align: center;">Aucun export récent</td></tr>';
    return;
  }
